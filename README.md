...
你可以像编写阻塞任务一样编写每个任务并使它们并发执行，且不需要 (rt)os 参与。  

你的 async 函数会经过脚本翻译状态机并与调度器对接，不需要手写状态机。  
函数可复用，故而可降低层次状态机心智负担且不再容易出现状态爆炸的情况。

目前紧耦合本人先前编写的 [ltx 调度器](https://github.com/TiX233/ltx)  
