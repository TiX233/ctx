...
你可以像编写同步任务一样编写每个异步任务并使它们并发执行，没有混乱流转回调，且不需要 (rt)os 参与。  

你的 async 函数会经过脚本翻译协程状态机并与调度器对接，不需要手写状态机。  
函数可复用，故而可降低层次状态机心智负担且不再容易出现状态爆炸的情况。

目前暂时紧耦合本人先前编写的 [ltx 事件调度器](https://github.com/TiX233/ltx)  
LTX-V4 提供高效的事件响应且低资源消耗，支持空闲休眠 tickless 操作，还支持同构多核 SMP 调度，自动均衡负载。
...
不需要修改编译器或者安装编译器插件，即使您不使用 async，这段代码也会退化到同步阻塞，业务还可以低痛移植到有 os 的项目上

目录说明：

* `examples`: 提供一些样例工程可供移植与编程参考
* `docs`: 供编程参考的文档
* `tests`: 翻译脚本测试用例
...
如果您有更强的 ai 工具或者想手写更受掌控的版本，那么我编写了一个 [源码翻译规则(模板)_V2.x.c](./源码翻译规则(模板)_V2.x.c)，您可以参考这个翻译规则来设计更好的脚本。
...
* events: 事件组。任务可以异步等待最多 30 个事件位；支持 **事件与** 以及 **事件或**；支持多生产者多消费者
* mutex: 互斥锁。用于任务间竞争硬件资源，可设置异步等待锁释放的超时时间
