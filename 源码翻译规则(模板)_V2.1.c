...
// 调度器回调，也就是翻译后的状态机代码
uint8_t _cocb_函数名(struct coro_stu *co){
    ...
    // label 必须顶格，便于区分
_colabel_下一步骤编号:
    /* BEGIN: 如果用户接收返回值，那么生成这部分 */
    #ifndef __ctx_customize_retval_被调用函数名
    用户接收返回值的变量 = ((struct _coval_被调用函数名 *)(co->son->prv_data))->_coretval_;
    #else
    用户接收返回值的变量 = __ctx_customize_retval_被调用函数名(co);
    #endif
    /* END: 如果用户接收返回值，那么生成这部分 */

    #ifndef __ctx_customize_no_free_被调用函数名
    // free 子协程对象
